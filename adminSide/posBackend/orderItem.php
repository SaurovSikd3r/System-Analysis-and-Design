                            ...
                            <div style="max-height: 40rem;overflow-y: auto;">
                                ...
                                if ($cart_result && mysqli_num_rows($cart_result) > 0) {
                                    while ($cart_row = mysqli_fetch_assoc($cart_result)) {
                                        ...
                                        echo '<td>Taka ' . number_format($item_price,2) . '</td>';
                                        echo '<td>' . $quantity . '</td>';
                                        echo '<td>Taka ' . number_format($total,2) . '</td>';
                                        // Check if the bill has been paid
                                        $payment_time_query = "SELECT payment_time FROM Bills WHERE bill_id = '$bill_id'";
                                        $payment_time_result = mysqli_query($link, $payment_time_query);
                                        $has_payment_time = false;
    ...
    <table class="table table-bordered ">
        <tbody>
            <tr>
                ...
                <td><?php echo number_format($cart_total, 2); ?> Taka</td>
            </tr>
            <tr>
                <td><strong>Cart Taxed</strong></td>
                <td><?php echo number_format($cart_total * $tax, 2); ?> Taka</td>
            </tr>
            <tr>
                <td><strong>Grand Total</strong></td>
                <td><?php echo number_format(($tax * $cart_total) + $cart_total, 2); ?> Taka</td>
            </tr>
        </tbody>
    </table>
</div>
